...
Your job is to parse a chronological log containing both fashion law changes and citizen sightings. 
You must identify every citizen who was in violation of the active laws at the moment they were sighted.

Your function takes a single input **logs** which is a multiline string that follow a strict format representing the chronological log.
...
1. Sighting Line: A person's name followed by a colon, then a comma-separated list of items they are wearing (any line that does not start with the above keywords)
